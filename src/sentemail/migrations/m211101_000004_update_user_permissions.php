...
class m211101_000004_update_user_permissions extends Migration
{
    ...
    {
        $this->updateSentEmailPermissions();

        // Add accessModule permissions
        $this->insert(Table::USERPERMISSIONS, [
            'name' => strtolower('sprout-module-sent-email:accessModule'),
        ]);
        ...
        $accessPluginPermissionId = (new Query())
            ...
            ->where([
                'name' => 'accessplugin-sprout-sent-email',
            ])
            ->scalar();
    ...
    public function updateSentEmailPermissions(): void
    {
        // Create the new Permission
        $this->insert(Table::USERPERMISSIONS, [
            'name' => strtolower('sprout-module-sent-email:viewSentEmail'),
        ]);
        ...
        $oldViewEmailPermissionIds = (new Query())
            ...
            ->where([
                'in', 'name', [
                    strtolower('sproutSentEmail-viewSentEmail'),
                    strtolower('sproutEmail-viewSentEmail'),
                ],
            ])
            ->column();
        ...
        $oldEditEmailPermissionIds = (new Query())
            ...
            ->where([
                'in', 'name', [
                    strtolower('sproutSentEmail-resendEmails'),
                    strtolower('sproutEmail-resendEmails'),
                ],
            ])
            ->column();
